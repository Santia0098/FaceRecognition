...
export const Pandemia = () => {
    ...
    const reconocimiento = async (video) => {
        const labels = ['cubrebocas', 'cubre', 'cubrea', 'no', 'no1', 'no2', 'no3'];
        ...
        const matcher = new faceapi.FaceMatcher(labelFaceDescriptions, 0.42);
        ...
        video.addEventListener('play', () => {
            ...
            setInterval(async () => {
                ...
                results.forEach((bestMach, i) => {
                    ...
                    if(text.substr(0, 7) === 'unknown' || text.substr(0, 7) === '' || text.substr(0,2) === 'no' || text.substr(0,3) === 'no1' || text.substr(0,3) === 'no2' || text.substr(0,3) === 'no3') {
                        const drawBox = new faceapi.draw.DrawBox(box, { label: text });
                        onChangeText('Sin cubrebocas');
                        drawBox.draw(canvas);
                    } else {
                        const drawBox = new faceapi.draw.DrawBox(box, { label: text });
                        onChangeText('Con cubrebocas');
                        drawBox.draw(canvas);
                    }    
                });
            }, 70);
                       ...
                       <div className="row">
                            ...
                            <div className="col-12">
                                <div className="fondo3" style={{ flexDirection: 'column' }}>
                                    <div className="mb-4">
                                    <Link to="/pandemia" > <span className="btn btn-primary " style={{width: '360px', height: '80px', marginTop: 4}}>Pandemia</span>  </Link>
                                    </div>
